...
This development auth server supports registration, login, profile management, avatar uploads
(local or S3), and server-side sessions persisted in SQLite.
...
- Sessions are stored server-side in the sessions table; the cookie contains a random sessionId only.
  - Users are persisted to SQLite so they survive server restarts.
